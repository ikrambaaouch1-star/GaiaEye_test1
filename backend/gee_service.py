...
def get_exhaustive_terroir_data(coords, start, end):
    """
    Extracts all requested raw data for the Terroir Intelligence Box in ONE GEE call.
    """
    roi = ee.Geometry.Rectangle([coords['west'], coords['south'], coords['east'], coords['north']])
    ...
    s2_indices = ee.Image.cat([
        ...
        s2_median.normalizedDifference(['B3', 'B8']).rename('NDWI'),
        s2_median.normalizedDifference(['B8', 'B11']).rename('NDMI')
    ])
    ...
    s1_coll = ee.ImageCollection('COPERNICUS/S1_GRD') \
               ...
               .filterDate(start, end) \
               .filter(ee.Filter.eq('instrumentMode', 'IW'))
    s1_mean = s1_coll.select(['VV', 'VH']).mean()
    
    # 3. Structural (GEDI)
    gedi = ee.ImageCollection("LARSE/GEDI/GEDI02_B_002") \
             .filterBounds(roi) \
             .select(['pai', 'fhd_normal', 'canopy_top_height']) \
             .mean()
             
    # 4. Climate (MODIS & GPM & CHIRPS)
    lst = get_modis_image(roi, start, end)
    chirps = get_chirps_image(roi, start, end)
    ...
    topo_img = ee.Image.cat([dem, slope.rename('slope'), aspect.rename('aspect')])

    # Consolidated Reduction
    # We use a dictionary to fetch everything in one go
    consolidated_tasks = ee.Dictionary({
        'opt_bands': s2_median.select(s2_bands).reduceRegion(ee.Reducer.mean(), roi, 20),
        'opt_indices': s2_indices.reduceRegion(ee.Reducer.mean(), roi, 10),
        'radar': s1_mean.reduceRegion(ee.Reducer.mean(), roi, 10),
        'lidar': gedi.reduceRegion(ee.Reducer.mean(), roi, 25),
        'climate_lst': lst.reduceRegion(ee.Reducer.mean(), roi, 1000),
        'climate_gpm': gpm.reduceRegion(ee.Reducer.mean(), roi, 10000),
        'climate_chirps': chirps.reduceRegion(ee.Reducer.mean(), roi, 5000),
        'topo': topo_img.reduceRegion(ee.Reducer.mean(), roi, 30)
    })
    
    print("Sending consolidated GEE request...")
    results = consolidated_tasks.getInfo()
    
    # Final data structuring
    return {
        'optical': {
            'bands': results.get('opt_bands', {}),
            'indices': results.get('opt_indices', {}),
            'metadata': {'resolution': '10m', 'source': 'Sentinel-2 L2A'}
        },
        'radar_lidar': {
            'vv': results.get('radar', {}).get('VV'),
            'vh': results.get('radar', {}).get('VH'),
            'rugosity': (results.get('radar', {}).get('VV', 0) or 0) / (results.get('radar', {}).get('VH', 1) or 1),
            'canopy_height': results.get('lidar', {}).get('canopy_top_height'),
            'biomass_index': results.get('lidar', {}).get('pai'),
            'vertical_profile': results.get('lidar', {}).get('fhd_normal')
        },
        'hyperspectral': {
            'bands_count': 240,
            'mineral_signatures': ['Kaolinite: Detected', 'Illite: Moderate', 'Iron Oxide: High'],
            'status': 'Simulated/Hyperspectral Proxy'
        },
        'climatology': {
            'lst_celsius': results.get('climate_lst', {}).get('constant', 20),
            'precip_gpm_mm': results.get('climate_gpm', {}).get('precipitationCal', 0),
            'precip_chirps_mm': results.get('climate_chirps', {}).get('precipitation', 0)
        },
        'topography': {
            'elevation': results.get('topo', {}).get('elevation'),
            'slope': results.get('topo', {}).get('slope'),
            'aspect': results.get('topo', {}).get('aspect'),
            'water_accumulation': (results.get('topo', {}).get('slope') or 0) * 0.5
        }
    }
